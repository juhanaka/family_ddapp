...
#Detech which relatinonship is considered
...
psql -d $DBNAME -c "drop table if exists permanent_tags_family_${relationship}_precision_is_correct;"

if [ -f "permanent_tags_family_${relationship}_precision_is_correct.sql" ]
  then
    psql -d $DBNAME < permanent_tags_family_${relationship}_precision_is_correct.sql
fi
...
psql -d $DBNAME -c \
"""
UPDATE permanent_tags_family_${relationship}_precision_is_correct
SET expectation = 
(SELECT f.expectation
FROM has_${relationship}_is_true_inference f
WHERE permanent_tags_family_${relationship}_precision_is_correct.sentence_id= f.sentence_id
AND   permanent_tags_family_${relationship}_precision_is_correct.description = f.description
LIMIT 1);
"""

#The table is exported and can then be synced with other people (through git for instance)
pg_dump $DBNAME -t permanent_tags_family_${relationship}_precision_is_correct > permanent_tags_family_${relationship}_precision_is_correct.sql
