...
if [ -f "$file_name" ]
  then
    perl -pi -e 's/tags_parent-precision_is_correct/tags_parent_precision_is_correct/' $file_name
...
if [ -f "$file_name_recall" ]
  then
    perl -pi -e 's/tags_parent-recall_is_correct/tags_parent_recall_is_correct/' $file_name_recall
    ...
    DELETE FROM permanent_tags_family_parent_precision_is_correct
    where sentence_id IN 
    (SELECT tags_parent_recall_is_correct.sentence_id FROM tags_parent_recall_is_correct 
    where permanent_tags_family_parent_precision_is_correct.description = tags_parent_recall_is_correct.description
    and tags_parent_recall_is_correct.is_correct != '');
    """
    ...
    SELECT t.relation_id, 
           ...
           'true',
           t.description
    FROM tags_parent_recall_is_correct t
    WHERE t.is_correct != '';
    """
fi
