...
if [ -f "$file_name" ]
  then
    perl -pi -e 's/tags_sibling-precision_is_correct/tags_sibling_precision_is_correct/' $file_name
...
if [ -f "$file_name_recall" ]
  then
    perl -pi -e 's/tags_sibling-recall_is_correct/tags_sibling_recall_is_correct/' $file_name_recall
    ...
    DELETE FROM permanent_tags_family_sibling_precision_is_correct
    where sentence_id IN 
    (SELECT tags_sibling_recall_is_correct.sentence_id FROM tags_sibling_recall_is_correct 
    where permanent_tags_family_sibling_precision_is_correct.description = tags_sibling_recall_is_correct.description
    and tags_sibling_recall_is_correct.is_correct != '');
    """
    ...
    SELECT t.relation_id, 
           ...
           'true',
           t.description
    FROM tags_sibling_recall_is_correct t
    WHERE t.is_correct != '';
    """
fi
